        ...
        NavigationStack {
            ...
            ScrollView {
                VStack(alignment: .leading) {
                    ...
                    ForEach(
                        contents.filter {
                            $0.majorCat.id == content.majorCat.id
                        }
                    ) { content in
                        HStack {
                            Text(content.name)
                            ...
                            // 북마크 버튼x
                            Button(action: {
                                if let current = bookmarkStates[content.id],
                                    current
                                {
                                    // 북마크 제거
                        ...
                        ForEach(0..<content.texts.count, id: \.self) { index in
                            let parts = content.texts[index].split(
                                separator: "*", maxSplits: 2,
                                omittingEmptySubsequences: false)
                            let separated = content.texts[index].components(
                                separatedBy: "***")
                            let imageFileName: String = separated.first ?? ""
                            let textContent =
                                separated.count > 1
                                ? separated[1] : content.texts[index]
            ...
            //            }
            .onAppear {
                for content in contents.filter({
                    $0.majorCat.id == self.content.majorCat.id
                }) {
